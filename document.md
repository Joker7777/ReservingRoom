...
1. 使用フロー
    1. 単体予約
        1. 新規登録/予約変更
            1. '〇'/予約のところをclick
            1. モーダル出現(初期値：名前=''、日付=click、時間=click、管理者名=''/DB値)
            1. 以上を入力、送信
            1. 日付被りをチェック ? DBにinsert/update, 完了 : 日付が被っている&管理者名
    1. 毎週予約
        1. 新規登録
            1. '〇'のところをclick
            1. モーダル出現(初期値：名前=''、日付=click、時間=click、管理者名=''/DB値)
            1. 毎週チェックボックスをチェック
            1. 期間(初期値：clickから1か月後まで)
        1. 予約変更
            1. 予約のところをclick
            1. モーダル出現(DB値)
            1. 時間の変更
            1. 送信ボタン
                1. この回のみ変更する(他の毎週予約から独立します)
                    1. 日付被りをチェック ? update, 完了 : 日付が被っている日程&管理者名
                    1. 毎週予約ID=nullとする
                1. 毎週予約全体の変更として保存する
                    1. 毎週予約IDで検索
...
* a
    ...
    * 20190129
        ...
        * スマホでのデザイン
            カレンダーの文字の大きさとかどうか？
            * 入力ホーム
                日付入力、日付と時間が改行してしまう => この際ラベルも改行すれば上から順に読める
                レスポンシブ調査
    ...
    * 20190203
        * editForm ok
            だけど毎週予約のときとかかぶりそうなので、APIでチェックは必須
        * branchわけ
        * 次回
            * migration
            * API
...
* さしあたってやる
    * 使用フローの明示 ok 20190412
    * 日付変更の復活 ok 20190413
    * DBの内容を読みこめていない ok 20190414
        store.jsがDBのカラム変更に対応していなかった,凡ミス
    * 毎週予約が失敗する
        500, 20190414.logを確認、対処
    * データの取得処理
        ちゃんとその週の予約のみ取得
    * updateの実装
    * 日付チェックの実装
